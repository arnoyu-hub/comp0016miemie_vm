"""
    Standalone file to analyse the results produced after running predict_publication.py on the HA_MODULE_LDA results and output the publications under each HA speciality
"""
...
def main() -> None:
    ...
    with open('main/NLP/LDA/HA_RESULTS/scopus_prediction_results.json', 'r') as f:
        results = json.load(f)
    ...
    for i in range(18):
        for doi, vals in results.items():
            if str(i) in vals:
                if float(vals[str(i)]) >= THRESHOLD:
                    lst[str(i)].append(doi)

    generate_csv(lst, cols, "main/NLP/LDA/HA_RESULTS/pub_analyse_20.csv")
