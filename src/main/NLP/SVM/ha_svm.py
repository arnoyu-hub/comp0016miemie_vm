...
class HaSvm(Svm):
    ...
    def write_results(self, X_test, y_test, results_file: str) -> None:
        ...
        MongoDbPusher().svm_ha_predictions(data)
    ...
    def print_predictions(self) -> None:
        ...
        for i in range(len(y_pred)):
            if i % 100 == 0:
                print('{}: HA probabilities = {}'.format(X_id[i], y_pred[i]))
    ...
    def print_text_prediction(self, text) -> None:
        ...
        for i in range(len(y_pred)):
            print('HA probabilities = {}'.format(y_pred[i]))
            print('HA = {}'.format(np.argmax(y_pred) + 1))
