...
class BubbleMongoSync():
    ...
    def __retrieve_publications_ha(self, limit: int) -> list:
        """
            Retrieves publication data and assignments from PostgreSQL database
        """
        ...
        cur = self.con.cursor()
        if limit:
            cur.execute('SELECT data, "assignedHA" FROM public.app_publicationha LIMIT {}'.format(limit))
        else:
            cur.execute('SELECT data, "assignedHA" FROM public.app_publicationha')
        result = cur.fetchall()
        return result
    ...
    def __bubble_exists_ha(self, x: int, y: int) -> int:
        """
            Checks if a bubble for a given approach ID (x) and speciality ID (y) exists in the PostgreSQL database
        """

        cur = self.con.cursor()
        query = """SELECT color_id from public.app_bubbleactha WHERE coordinate_speciality_id = \'{0}\' AND coordinate_approach_id = \'{1}\'""".format(y, x)
        cur.execute(query)
        return len(cur.fetchall()) != 0
    ...
    def __update_bubbles_ha(self, bubble_dict: dict) -> None:
        """
            Updates the PostgreSQL database bubbles with a new list of people and creates a new bubble if it doesn't already exist 
        """
        ...
        for i in bubble_dict:
            ...
            if len(bubble_dict[i]['list_of_people']) != 0:
                ...
                if not self.__bubble_exists_ha(elem[0], elem[1]):
                    query = "INSERT INTO public.app_bubbleactha (coordinate_speciality_id, coordinate_approach_id, color_id, list_of_people) VALUES({0}, {1}, {2}, \'{3}\')".format(
                        elem[1], elem[0], bubble_dict[i]['color'], list_of_ppl)
                else:
                    query = """UPDATE public.app_bubbleactha SET coordinate_speciality_id = \'{0}\', coordinate_approach_id = \'{1}\', color_id = \'{2}\', list_of_people = \'{3}\' WHERE coordinate_speciality_id = {0} AND coordinate_approach_id = {1}""".format(
                        elem[1], elem[0], bubble_dict[i]['color'], list_of_ppl)
    ...
    def __create_bubble_data_ha(self, pubs: list) -> dict:
        """
            Creates the data for each bubble in preparation for updating the PostgreSQL database
        """
        ...
        self.__update_bubbles_ha(new_bubble)
    
    def run(self) -> None:
        #data_publications = self.__retrieve_publications(limit=None)
        data_publications_ha = self.__retrieve_publications_ha(limit=None)
        #self.__create_bubble_data(data_publications)
        self.__create_bubble_data_ha(data_publications_ha)
